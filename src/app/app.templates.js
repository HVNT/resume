angular.module('hb.app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('/app/projects/views/projects.list.project.html',
    "<div hb-s-row hb-s-pxy=8><div hb-s-row><div hb-s-col=12><img class=inline-block ng-src={{activeProject.forLogoUrl}} height=50px hb-s-pr=3 hb-s-pt=1><div hb-s-row hb-s-pb=2><span>{{activeProject.title}}</span></div><div hb-s-row><span>{{activeProject.subtitle}}</span></div></div></div><div hb-s-row></div></div>"
  );
